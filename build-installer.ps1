# CallTrack KARAHAN - Tek kurulum EXE olusturma
# Cozumun kok dizininde calistirin: .\build-installer.ps1

$ErrorActionPreference = "Stop"
$Root = $PSScriptRoot
$WebProj = Join-Path $Root "CallTrackKARAHAN.Web\CallTrackKARAHAN.Web.csproj"
$InstallerProj = Join-Path $Root "CallTrackKARAHAN.Installer\CallTrackKARAHAN.Installer.csproj"
$PayloadDir = Join-Path $Root "CallTrackKARAHAN.Installer\_payload"
$PayloadZip = Join-Path $Root "CallTrackKARAHAN.Installer\payload.zip"

Write-Host "CallTrack KARAHAN - Kurulum EXE olusturuluyor..." -ForegroundColor Cyan
...
$WebKurulumPs1 = Join-Path $Root "CallTrackKARAHAN.Web\Kurulum.ps1"
$WebKurulumBat = Join-Path $Root "CallTrackKARAHAN.Web\Kurulum.bat"
...
$TempOut = Join-Path $Root "CallTrackKARAHAN.Installer\bin\SetupOutput"
if (Test-Path $TempOut) { Remove-Item $TempOut -Recurse -Force }
dotnet publish $InstallerProj -c Release -r win-x64 --self-contained true -p:PublishSingleFile=true -o $TempOut
...
# EXE'yi proje kokune kopyala
$SetupExeName = "CallTrackKARAHAN_Setup.exe"
$SetupExe = Join-Path $Root $SetupExeName
Copy-Item (Join-Path $TempOut $SetupExeName) -Destination $SetupExe -Force

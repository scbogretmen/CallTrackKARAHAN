...
const string HedefKlasor = @"C:\CallTrackKARAHAN";

// Yonetici degilse kendini yonetici olarak yeniden baslat
if (!IsRunAsAdministrator())
{
    Console.WriteLine("Yonetici izni isteniyor...");
    var exe = Environment.ProcessPath ?? AppContext.BaseDirectory.TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar) + Path.DirectorySeparatorChar + "CallTrackKARAHAN_Setup.exe";
    try
    {
...
Console.WriteLine("CallTrack KARAHAN kurulumu basliyor...");

// Gomulu payload.zip'i bul ve C:\CallTrackKARAHAN'e cikart
...
{
    Console.WriteLine("HATA: Kurulum paketi (payload) bulunamadi. CallTrackKARAHAN_Setup.exe bozuk veya eksik olabilir.");
    Bekle();
    return 1;
}
...
try
{
    tempZip = Path.Combine(Path.GetTempPath(), "CallTrackKARAHAN_payload_" + Guid.NewGuid().ToString("N") + ".zip");
